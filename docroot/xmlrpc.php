...
function & XML_serialize(&$data, $level = 0, $prior_key = NULL){
	...
	while(list($key, $value) = each($data)){
		...
		if(false === strpos($key, ' attr')){ #if it's not an attribute
...
class XML
{
    ...
    public function __construct($data = null)
    {
        ...
        xml_set_character_dataHandler($this->parser, 'data');
	...
	if(is_array($data)){
		...
		if(!$type){
			if(is_int($data)){
				...
				$returnvalue['string'] = htmlspecialchars($data, ENT_QUOTES | ENT_HTML5);
				return $returnvalue;
			}
		}else{
			$returnvalue[$type] = htmlspecialchars($data, ENT_QUOTES | ENT_HTML5);
		}
	}
	return $returnvalue;
}
...
function XMLRPC_adjustValue(&$current_node){
	if(is_array($current_node)){
		if(isset($current_node['array'])){
			...
			$types = ['string', 'int', 'i4', 'double', 'dateTime.iso8601', 'base64', 'boolean'];
			$fell_through = true;
...
function XMLRPC_request($site, $location, $methodName, $params = NULL, $user_agent = NULL){
	...
	if(!$conn){ #if the connection was not opened successfully
		...
		return [false, ['faultCode' => 10532, 'faultString' => "Connection failed: Couldn't make the connection to $site."]];
	}else{
		$headers =
		...
		if(isset($data['methodResponse']['fault'])){
			$return = [false, XMLRPC_adjustValue($data['methodResponse']['fault']['value'])];
			if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
				XMLRPC_debug('XMLRPC_request', "<p>Returning:</p>\n\n" . XMLRPC_show($return, 'var_dump', true));
			}
			return $return;
		}else{
			$return = [true, XMLRPC_adjustValue($data['methodResponse']['params']['param']['value'])];
			if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
				XMLRPC_debug('XMLRPC_request', "<p>Returning:</p>\n\n" . XMLRPC_show($return, 'var_dump', true));
			}
			return $return;
...
function XMLRPC_debug($function_name, $debug_message){
	$GLOBALS['XMLRPC_DEBUG_INFO'][] = [$function_name, $debug_message];
}
...
function XMLRPC_show($data, $func = 'print_r', $return_str = false){
	...
	if($return_str){
		...
		echo '<pre>', htmlspecialchars($output, ENT_QUOTES | ENT_HTML5), "</pre>\n";
	}
}
